...
namespace SpeechAgent.Services
{
  ...
  ) : IPatientSearchService
  {
    ...
    private bool FindWindowByTitle(out bool isNewCreated)
    {
      ...
      LogUtils.WriteLog(LogLevel.Debug, $"[FindWindowByTitle] 진입 - TargetAppName={Settings.TargetAppName}, IsWindowValid={isWindowValid}");
      if (!isWindowValid)
      {
        if (IsCustom)
        {
          ...
          {
            LogUtils.WriteLog(LogLevel.Debug, $"[FindWindowByTitle] Custom 윈도우 탐색 실패 - CustomExeTitle={Settings.CustomExeTitle}");
            return false;
          }
        }
        else
        {
          ...
          {
            case AppKey.USarang:
              var usarangFound = _searcher.FindWindowByTitles("툴버전");
              LogUtils.WriteLog(LogLevel.Debug, $"[FindWindowByTitle] USarang FindWindowByTitles(툴버전) 결과: {usarangFound}");
              if (!usarangFound)
                return false;
              break;
            case AppKey.Brain:
    ...
    private AutomationAppControls? GetAppControls(List<AutomationControlInfo> controls)
    {
      ...
      LogUtils.WriteLog(LogLevel.Debug, $"[GetAppControls] {Settings.TargetAppName} 처리 후 - result null여부={result == null}, ChartTextBox null여부={result?.ChartTextBox == null}, NameTextBox null여부={result?.NameTextBox == null}");
    ...
    {
      LogUtils.WriteLog(LogLevel.Debug, $"[FindUSarangControls] 진입 - controls.Count={controls.Count}");
      ...
      {
        LogUtils.WriteLog(LogLevel.Debug, "[FindUSarangControls] editControls가 2개 미만 - null 반환");
        return null;
      }
      ...
      LogUtils.WriteLog(LogLevel.Debug, $"[FindUSarangControls] chartControl null여부={chartControl == null}, nameControl null여부={nameControl == null}");

      if (chartControl == null || nameControl == null)
      {
        LogUtils.WriteLog(LogLevel.Debug, "[FindUSarangControls] chartControl/nameControl 중 하나가 null - null 반환");
        return null;
      }
      ...
      try
      {
        ...
        LogUtils.WriteLog(LogLevel.Debug, $"[FindUSarangControls] OCR 처리 - hWnd={hWnd}");
        if (hWnd != IntPtr.Zero)
        {
          // 윈도우의 절대 좌표 얻기
          ...
          LogUtils.WriteLog(LogLevel.Debug, $"[FindUSarangControls] OCR 캡처 영역 - rect={chartRectRelative}, dpiScale={dpiScale}");

          var chartImg = _windowCaptureService.CaptureWindow(hWnd, chartRectRelative);
          LogUtils.WriteLog(LogLevel.Debug, $"[FindUSarangControls] CaptureWindow - chartImg null여부={chartImg == null}");
          ...
          {
            LogUtils.WriteLog(LogLevel.Debug, "[FindUSarangControls] 이전 이미지와 동일 - 캐시 사용");
            return _appControls;
          }
          ...
          if (chartImg != null)
          {
            ...
            LogUtils.WriteLog(LogLevel.Debug, $"[FindUSarangControls] OCR 결과 - chartText='{chartControl.Text}', previousChart='{previousChart}'");
          ...
          {
            LogUtils.WriteLog(LogLevel.Debug, "[FindUSarangControls] chartImg가 null - null 반환");
    ...
    public async Task<PatientInfo> FindPatientInfo()
    {
      ...
      {
        LogUtils.WriteLog(LogLevel.Debug, "[FindPatientInfo] FindWindowByTitle 실패 - 빈 PatientInfo 반환");
        _searcher.ClearFoundControls();
        _appControls.ClearControls();
        _previousImageResult.Clear();
        return new PatientInfo();
      }
      LogUtils.WriteLog(LogLevel.Debug, $"[FindPatientInfo] FindWindowByTitle 성공 - isNewCreated={isNewCreated}, UseCustomUserImage={_settingsService.UseCustomUserImage}");
      ...
      else
      {
        ...
        LogUtils.WriteLog(LogLevel.Debug, $"[FindPatientInfo] controls 로드 - 캐시사용={useCached}, count={controls.Count}");

        // 차트, 수진자명 컨트롤 찾아서 전달
        var appControlsResult = GetAppControls(controls);
        LogUtils.WriteLog(LogLevel.Debug, $"[FindPatientInfo] GetAppControls 결과 - null여부={appControlsResult == null}, ChartTextBox={(_appControls.ChartTextBox != null)}, NameTextBox={(_appControls.NameTextBox != null)}");

        var patientInfo = CreatePatientInfo();
        LogUtils.WriteLog(LogLevel.Debug, $"[FindPatientInfo] CreatePatientInfo 결과 - Chart='{patientInfo.Chart}', Name='{patientInfo.Name}'");
        return patientInfo;
      }
    }
